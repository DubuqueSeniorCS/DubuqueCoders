...
void setupSerial() 
{
  ...
  Serial.println("Enter LED Brightness 0 to 7 or 'c' to clear or 'f' to flicker or 'rgb' to set color");
}
...
void loop() {
  ...
  if (Serial.available())
  {
    ...
    if (ch == 'b')
    {
      ...
      if (ch >= '0' && ch <= '9')
      {
        byte color_of_red = ch - '0';
        color_of_red = color_of_red * 28;
        flames[0].rgb[0] = color_of_red;
        ch = Serial.read();
        byte color_of_green = ch - '0';
        flames[0].rgb[1] = color_of_green;
        color_of_green = color_of_green * 28;
        ch = Serial.read();
        byte color_of_blue = ch - '0';
        color_of_blue = color_of_blue * 28;
        flames[0].rgb[2] = color_of_blue;
                
        Serial.print("set color to ");
        Serial.print("r:");
        Serial.print(color_of_red);
        Serial.print("g:");
        Serial.print(color_of_green);
        Serial.print("b:");
        Serial.println(color_of_blue);
      }
      ch='t';
    }
    else
    {
      if (ch >= '0' && ch <= '7')
      {
        commanded_brightness = ch - '0';
        flames[0].state = -1;
        UpdateFlameColor(0, commanded_brightness*36);
        Serial.print("Turned on LED at commanded_brightness");
        Serial.println(commanded_brightness);
      }
      if (ch == 'c')
      {
        flames[0].state = -1;
